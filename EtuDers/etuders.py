...
import requests
from etudersDB import db, LoggingEnabled, Log, OgrenciLog, Ogrenci, DersBilgi, DersKayit
...
def ogrenciDersProgramiGetir(ogrenciNo, ipAdres):
    dersListesi = db.session.query(Ogrenci.ogrenciNo).filter_by(ogrenciNo=ogrenciNo).all
